...
<div :class="`relative w-full overflow-hidden ${className}`" :clientWidth="carouselWidth">
  ...
  <template v-if="dots">
    <div class="flex justify-center mt-20px">
      <template v-for="(item, i) in items">
        <button :class="`hover:opacity-75 ${i === activeIndex ? '' : 'opacity-50'}`" @click="() => goTo(i)" :key="i">
...
export default defineComponent({
  ...
  setup(props) {
    const { items, startIndex, autoplay, loop, dots, controls, total, onChange } = toRefs(props);
    const className = ref("");
    ...
    return { items, startIndex, autoplay, loop, dots, controls, total, className, onChange, activeIndex, carouselWidth, intervalCarousel, goTo }
  }
});
</script>